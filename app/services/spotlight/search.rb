module Spotlight
  class Search
    ...
    def fetch_results
      ...
      ApiLogging::Logger.fatal("Testing 1A START | #{ENV.fetch('SPOTLIGHT_USERNAME', nil)}, #{ENV.fetch('SPOTLIGHT_PASSWORD', nil)}, #{ENV.fetch('SPOTLIGHT_CLIENT_ID', nil)}, #{ENV.fetch('SPOTLIGHT_CLIENT_SECRET', nil)}, #{ENV.fetch('SPOTLIGHT_AUTH_URL', nil)} | method:fetch_results, #{e.to_json}")
