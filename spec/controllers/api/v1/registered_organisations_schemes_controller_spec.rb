...
RSpec.describe Api::V1::RegisteredOrganisationsSchemesController, type: :controller do
  ...
  describe 'search_organisation_by_scheme' do
    let(:clientid) { ENV['CLIENT_ID'] }
    let(:ccs_org_id) { nil }
    let(:id) { nil }
    let(:scheme) { nil }
    ...
    context 'when authorized' do
      ...
      context 'when not found' do
        it 'returns 404' do
          get :search_organisation, params: { id: 123456, scheme: 'GB-COH', clientid: clientid }
          expect(response).to have_http_status(:not_found)
        end
      end
      ...
      context 'when invalid scheme' do
        it 'returns 400' do
          get :search_organisation, params: { scheme: 'HB-CHH', id:123456, clientid: clientid }
          expect(response).to have_http_status(:bad_request)
        end
      end
    end
