...
RSpec.describe ApiValidations::Scheme, type: :model do
  describe 'validations' do
    ...
    context 'when all params are present' do
      it 'is valid' do
        expect(described_class.new({ id: organisation_scheme_identifier.scheme_org_reg_number, scheme: scheme_register.scheme_register_code, organisation_id: organisation_scheme_identifier.organisation_id }).valid?).to eq true
      end
    end
    ...
    context 'when scheme is invalid' do
      it 'is not valid' do
        expect { described_class.new({ id: organisation_scheme_identifier.scheme_org_reg_number, scheme: 'invalid', organisation_id: organisation_scheme_identifier.organisation_id }).valid? }.to raise_exception(ApiValidations::ApiError)
      end
    end
    ...
    context 'when organisation_id is invalid' do
      it 'is not valid' do
        expect { described_class.new({ id: organisation_scheme_identifier.scheme_org_reg_number, scheme: scheme_register.scheme_register_code, organisation_id: 'invalid' }).valid? }.to raise_exception(ApiValidations::ApiError)
      end
    end
    ...
    context 'when organisation_id is from a different organisation_scheme' do
      it 'is not valid' do
        expect { described_class.new({ id: organisation_scheme_identifier.scheme_org_reg_number, scheme: scheme_register.scheme_register_code, organisation_id: organisation_scheme_identifier1.organisation_id }).valid? }.to raise_exception(ApiValidations::ApiError)
      end
    end
  end
end
