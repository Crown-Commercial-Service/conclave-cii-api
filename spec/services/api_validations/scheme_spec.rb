...
RSpec.describe ApiValidations::Scheme, type: :model do
  describe 'validations' do
    ...
    let(:organisation_scheme_identifier) { create(:organisation_scheme_identifier, scheme_code: scheme_register.scheme_register_code) }
    let(:organisation_scheme_identifier1) { create(:organisation_scheme_identifier) }

    context 'when all params are present' do
      it 'is valid' do
        expect(described_class.new({ id: organisation_scheme_identifier.scheme_org_reg_number, scheme: scheme_register.scheme_register_code, ccs_org_id: organisation_scheme_identifier.ccs_org_id }).valid?).to be true
      end
    end
