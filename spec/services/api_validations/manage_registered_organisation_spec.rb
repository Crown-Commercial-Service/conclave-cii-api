...
RSpec.describe ApiValidations::ManageRegisteredOrganisation, type: :model do
  describe 'validations' do
    ...
    context 'when organisationId is present' do
      it 'is valid' do
        expect(described_class.new({ identifier: organisation_params, 'identifier' => organisation_params, organisationId: organisation_scheme_identifier.organisationId }).valid?).to eq true
      end
    end

    context 'when organisationId is invalid' do
      it 'is not valid' do
        expect { described_class.new({ organisationId: 'test' }).valid? }.to raise_exception(ApiValidations::ApiError)
      end
    end
