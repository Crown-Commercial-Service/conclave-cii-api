...
RSpec.describe ApiValidations::RemoveScheme, type: :model do
  describe 'validations' do
    ...
    context 'when all params are present' do
      it 'is valid' do
        expect(described_class.new({ id: organisation_scheme_identifier.scheme_org_reg_number, scheme: scheme_register.scheme_register_code }).valid?).to eq true
      end
    end
    ...
    context 'when id is missing' do
      it 'is valid' do
        expect(described_class.new({ scheme: scheme_register.scheme_register_code }).valid?).to eq true
      end
    end
  end
end
