...
RSpec.describe ApiValidations::RemoveOrganisationAdditionalIdentifier, type: :model do
  describe 'validations' do
    ...
    context 'when all params are present' do
      it 'is valid' do
        expect(described_class.new({ id: 24325263, scheme: 'GB-COH', organisation_id: organisation_scheme_identifier.organisation_id }).valid?).to eq true
      end
    end

    context 'when identifier is missing' do
      it 'is not valid' do
        expect { described_class.new({ organisation_id: organisation_scheme_identifier.organisation_id }).valid? }.to raise_exception(ApiValidations::ApiError)
      end
    end

    context 'when organisation_id is missing is missing' do
      it 'is not valid' do
